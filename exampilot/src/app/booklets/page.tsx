  ...
  if (Object.keys(booklets).length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center bg-white rounded-3xl border border-gray-100 shadow-sm">
        <Inbox size={40} strokeWidth={1.5} className="mb-3 opacity-50 text-gray-400" aria-hidden="true" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Object.entries(subjects).map(([subject, count]) => (
              <Link
                ...
                className="group flex flex-col justify-between p-6 rounded-3xl bg-white border border-gray-100 shadow-sm hover:border-brand-accent-500 hover:shadow-md transition-all active:scale-[0.98] min-h-[160px] relative overflow-hidden"
              >
                {/* Subtle gradient flair */}
                <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-amber-50 to-transparent rounded-bl-full opacity-50 group-hover:scale-110 transition-transform" />
                
                <div className="relative z-10">
                  <h3 className="text-xl font-black text-gray-900 tracking-tight group-hover:text-brand-accent-600 transition-colors">
                    {subject} Practice Booklet
                  </h3>
                  <p className="text-xs font-bold uppercase tracking-widest text-brand-accent-600 mt-1">
                    {examTarget} Mock
                  </p>
                </div>
                ...
                <div className="relative z-10 flex items-center justify-between mt-6 pt-4 border-t border-gray-50">
                  <div className="flex items-center gap-1.5 text-sm font-medium text-gray-500">
                    <FileText size={14} strokeWidth={1.75} aria-hidden="true" /> {count} Questions
                  </div>
                  <span className="w-8 h-8 rounded-full bg-amber-50 text-brand-accent-600 flex items-center justify-center text-sm font-bold group-hover:bg-brand-accent-500 group-hover:text-white transition-colors">
                    →
                  </span>
                </div>
              </Link>
    ...
    <div className="flex flex-col gap-6 p-4 pt-6 pb-24 max-w-5xl mx-auto">
      ...
      <section className="mb-6">
        ...
        >
          ...
          <div className="absolute inset-0 bg-[linear-gradient(to_right,#F5A62315_1px,transparent_1px),linear-gradient(to_bottom,#F5A62315_1px,transparent_1px)] bg-[size:24px_24px] pointer-events-none" />
          <div className="absolute top-0 right-0 w-64 h-64 bg-amber-500/20 rounded-full blur-3xl group-hover:bg-amber-500/30 transition-colors pointer-events-none" />
          
          <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
            <div className="md:col-span-2 flex items-start gap-4">
              <div className="w-14 h-14 rounded-2xl bg-amber-500/20 text-amber-400 flex items-center justify-center shrink-0 backdrop-blur-sm border border-amber-500/30">
                <Newspaper size={28} strokeWidth={1.75} />
              </div>
              <div>
                <div className="flex flex-wrap gap-2 mb-2">
                  <span className="px-3 py-1 bg-amber-950/50 text-amber-400 text-[10px] font-bold uppercase tracking-widest rounded-lg border border-amber-500/30">
            ...
            <div className="md:col-span-1 flex items-center gap-3 justify-start md:justify-end">
              <span className="px-4 py-2 bg-amber-500/10 text-amber-300 text-sm font-bold rounded-xl border border-amber-500/20">
                Auto-Updating
              </span>
              <div className="w-12 h-12 rounded-xl bg-white text-slate-900 flex items-center justify-center font-bold text-lg group-hover:bg-brand-accent-500 group-hover:text-brand-accent-ink transition-colors">
                →
              </div>
            </div>
          </div>
