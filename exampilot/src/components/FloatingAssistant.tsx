...
import { Plane, ChevronDown, X, AlertTriangle } from "lucide-react";
...
export default function FloatingAssistant() {
  ...
  return (
    <div className="print:hidden fixed right-4 md:right-6 z-50 flex flex-col items-end bottom-[calc(var(--nav-height)+env(safe-area-inset-bottom)+1rem)] md:bottom-6">
      ...
      >
        {/* Header */}
        <div className="relative bg-gradient-to-br from-brand-bg-elevated to-brand-bg-surface p-4 text-white flex items-center justify-between shrink-0 overflow-hidden border-b border-brand-border-subtle">
          {/* subtle radar sweep accent */}
          <div className="pointer-events-none absolute -right-6 -top-8 w-28 h-28 rounded-full bg-brand-accent-500/10 blur-xl" aria-hidden="true" />
          <div className="flex items-center gap-3 relative z-10">
            <div className="w-10 h-10 bg-brand-accent-500/15 ring-1 ring-brand-accent-500/30 rounded-2xl flex items-center justify-center shadow-inner">
              <Plane size={18} strokeWidth={1.75} className="text-brand-accent-500" />
            </div>
            <div>
              <h3 className="font-black text-base leading-tight tracking-tight">Tejas</h3>
              <p className="text-[10px] text-brand-ink-muted uppercase tracking-widest font-bold flex items-center gap-1.5">
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 scroll-smooth bg-slate-50">
              ...
              <div
                className={`px-3.5 py-2.5 rounded-2xl text-sm leading-relaxed overflow-hidden shadow-sm ${
                  msg.role === "user"
                    ...
                    : "bg-white border border-slate-200 text-slate-800 rounded-bl-sm prose prose-sm prose-slate max-w-none prose-p:leading-snug prose-p:my-1 prose-ul:my-1 prose-li:my-0.5 prose-strong:text-amber-700"
                }`}
              >
                {msg.role === "assistant" ? (
          ...
          {error && (
            <div className="self-start w-full bg-red-50 border border-red-200 rounded-2xl p-3 flex items-start gap-3">
              <AlertTriangle size={18} strokeWidth={1.75} className="text-red-500 flex-shrink-0" aria-hidden="true" />
              ...
              <div className="flex flex-wrap gap-2">
                {QUICK_PROMPTS.map((q) => (
                  <button
                    ...
                    className="text-xs font-semibold text-amber-700 bg-white border border-amber-200 hover:border-amber-400 hover:bg-amber-50 px-3 py-1.5 rounded-full transition-all active:scale-95 shadow-sm"
                  >
                    {q.label}
                  </button>
                ))}
        ...
        <form onSubmit={onFormSubmit} className="p-3 bg-white border-t border-slate-100 flex items-center gap-2 shrink-0">
          <input
            ...
            className="flex-1 bg-slate-100 text-slate-800 placeholder:text-slate-400 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/50"
            disabled={isLoading}
          />
          {isLoading ? (
            <button
            ...
            <button
              ...
              className="w-10 h-10 flex items-center justify-center bg-brand-accent-500 text-brand-accent-ink rounded-xl hover:bg-brand-accent-400 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-95"
            >
              ↑
            </button>
          )}
      ...
      >
        {!isOpen && (
          <span className="absolute inset-0 rounded-full bg-brand-accent-500/40 animate-ping" style={{ animationDuration: "2.5s" }} aria-hidden="true" />
        )}
        <span className="relative z-10">{isOpen ? <ChevronDown size={20} strokeWidth={1.75} /> : <Plane size={20} strokeWidth={1.75} />}</span>
      </button>
    </div>
  );
}
